...
(10) unsigned NOT NULL,
   ...
   PRIMARY KEY
(`id`),
   CONSTRAINT `fk_meal_id` FOREIGN KEY
...
(1),
  ...
  PRIMARY KEY
(`id`),
  CONSTRAINT `fk_meal_review_id` FOREIGN KEY
(`meal_id`) REFERENCES `meal`
(`id`) ON
DELETE CASCADE
) ENGINE=InnoDB
...
('Green menu', 	'White Asparagus- Onions - Spinach - Cep Mushrooms 
...
SELECT m.id                                         AS meal_id,
	m.title,
	m.max_reservations,
	Sum(r.number_of_guests)                      AS total_reservations,
	m.max_reservations - Sum(r.number_of_guests) AS available_meals
FROM meal m,
	reservation r
WHERE m.id = r.meal_id
GROUP BY m.id
HAVING m.max_reservations - Sum(r.number_of_guests) > 0;

-- Get meals that partially match a title. Rød grød med will match the meal with the title Rød grød med fløde
SELECT *
FROM meal
WHERE meal.title LIKE '%pizza' OR meal.title LIKE 'pizza%';

-- Get meals that has been created between two dates
SELECT *
FROM meal
WHERE meal.created_date 
...
SELECT Avg(r.stars) AS avg_review,
	m.title
FROM meal m,
	review r
WHERE  m.id = r.meal_id
...
SELECT meal.id, meal.title, AVG(review.stars) AS review_average
FROM meal
